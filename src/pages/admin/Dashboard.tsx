  ...
  const generateInsights = () => {
    ...
    if (topProducts.length > 0 && totalUnits > 0) {
      ...
      if (pct > 25) {
        insights.push({
          type: "info",
          text: t("insight_top_driver", { name: topProd.name, pct: pct.toFixed(0) }),
        });
      }
    }
  ...
  const handleExport = () => {
    try {
      if (rawOrders.length === 0) {
        toast.info(t("no_orders_to_export"));
        return;
      }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm flex flex-col gap-2">
          <div className="flex items-center gap-3 text-stone-500 mb-1">
            ...
            <span className="font-medium">{t("avg_order_value")}</span>
          </div>
          <div className="text-[var(--color-primary)] font-bold leading-tight">
          ...
          <p className="text-sm font-medium text-stone-500">{t("per_order")}</p>
        </div>
      </div>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-red-500 font-medium">
                      {t("left_in_stock", { count: prod.stockCount })}
                    </p>
                  </div>
                  <Link
                    to="/admin/inventory"
                    ...
                    >
                      <div>
                        <p className="font-medium text-[var(--color-primary)]">
                          {order.customerName || t("not_available")}
                        </p>
