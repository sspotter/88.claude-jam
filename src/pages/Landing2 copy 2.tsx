...
export default function Landing2() {
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      const heroEl = document.querySelector(".an-hero") as HTMLElement | null;
      if (heroEl) {
        const heroH = heroEl.offsetHeight;
        // progress: 0 at top, 1 when hero is fully scrolled past
        const hp = Math.max(0, Math.min(1, lastY / (heroH * 0.65)));
        heroEl.style.setProperty("--hero-p", String(hp));
      }
    ...
    <>
      <style>{`
        ...
        .an-hero-video {
          ...
          /* start dimmed; the vignette fading out reveals full brightness naturally */
          filter: brightness(calc(0.45 + var(--hero-p) * 0.55)) contrast(1.1);
        }
        .an-hero-vignette {
          position: absolute; inset: 0; pointer-events: none;
          background: radial-gradient(ellipse at 50% 60%, transparent 30%, rgba(13,13,13,0.85) 100%);
          /* fade the dark overlay out as hero-p increases */
        ...
        .an-hero-content {
          ...
          /* dissolve upward as hero-p increases */
